...
//Rectangle{
        ...
        Menu {
            ...
            MenuItem {
                action: Action {
                    ...
                    text: "文字颜色";
                    onTriggered: doc.selectColor(doc.onTextColorSelected);
                }
            }
            MenuItem {
                action: Action{
                    ...
                    text: "文字背景色";
                    onTriggered: doc.selectColor(doc.onTextBackgroundColorSelected);
                }
            }
            MenuItem {
                action: Action{
    ...
    Item {
        ...
        TextArea {
            ...
            font.pointSize: 12;
            style: TextAreaStyle{
                backgroundColor: doc.textBackgroundColor;
    ...
    function onTextColorSelected(){
        doc.textColor = colorDlg.color;
    }

    function onTextBackgroundColorSelected(){
        doc.textBackgroundColor = colorDlg.color;
    }
    ...
    FileDialog {
        ...
        onAccepted: {
            ...
            if(Qt.platform.os == "windows"){
                doc.statusBar.text = filepath.slice(8);
            }else{
                doc.statusBar.text = filepath.slice(7);
            }
            var dot = filepath.lastIndexOf(".");
            var sep = filepath.lastIndexOf("/");
            if(dot > sep){
                var ext = filepath.substring(dot);
                doc.processFile(fileUrl, ext.toLowerCase());
            }else{
                doc.statusBar.text = "Not Supported!";
            }
        }
    }
}
